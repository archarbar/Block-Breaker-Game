	...
	public List<TOGridCell> getBlocksAtCurrentLevel() {
		...
		if (error == "") {
			try {
				...
				JOptionPane.showMessageDialog(null, "Successfully positionned block at level: " + level);
			} catch (InvalidInputException e) {
				error = e.getMessage();
				JOptionPane.showMessageDialog(null, error);
			}
...
//			for (TOGridCell block: blocks ) {
			for(int i =0; i<10 ;i++) {
				int y = 20 + 20*i;
				g.fillRect(20 + y, 20 ,20 ,20);				
				g.setColor(Color.blue);
				
			}
//				//create new block 
//				g.setColor(new Color(block.getRed(),block.getGreen(),block.getBlue()));
//				g.fillRect(block.getGridHorizontalPosition(), block.getGridVerticalPosition(), boxSize,boxSize);
//				//do the points
//			}
